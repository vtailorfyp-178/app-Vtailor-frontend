...
const sareeStyles = [
  ...
  { id: 'frill', name: 'Frill saree', image: frillSareeImage },
] as const;

export default function SareeStyleScreen() {
  ...
  const modelName = (params.modelName as string) || 'Saree';

  return (
    <ThemedView style={styles.container}>
      <View style={[styles.header, { backgroundColor: tint }]}>
        <AppBackButton onPress={() => (router as any).back()} variant="tint" />
        <ThemedText style={styles.headerTitle}>Saree style</ThemedText>
        <View style={styles.headerSpacer} />
      </View>

      <ScrollView contentContainerStyle={styles.grid}>
        {sareeStyles.map((style) => (
          <Pressable
            key={style.id}
            onPress={() =>
              (router as any).push({
