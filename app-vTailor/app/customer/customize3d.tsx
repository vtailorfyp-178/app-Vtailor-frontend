...
/** Long frock only: omit these from Colors tab (other dress types unchanged). */
...
function stripLongFrockDisallowedColors(
  ...
  if (s.colors != null && LONG_FROCK_EXCLUDED_COLORS.has(s.colors))
...
export default function Customize3D() {
  ...
  const customizationColors = useMemo(() => {
    if (modelId === 'long-frock')
      return colorOptions.filter((c) => !LONG_FROCK_EXCLUDED_COLORS.has(c.id));
    return colorOptions;
  }, [modelId]);
  ...
  const selectedColor = customizationColors.find((opt) => opt.id === selections.colors)?.name;
