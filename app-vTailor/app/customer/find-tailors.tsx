...
const MapView = mapModule?.default as any;
const Marker = mapModule?.Marker as any;
    ...
    <ThemedView style={styles.container}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsRow}>
        ...
        {specialtyOptions
          ...
          .slice(0, 5)
          .map((item) => renderFilterChip(item, specialty === item, () => setSpecialty(item)))}
      ...
      {!loading && !errorText && viewMode === 'map' && region && Platform.OS !== 'web' && MapView && Marker ? (
      ...
      ) : !loading && !errorText && viewMode === 'map' && region && Platform.OS === 'web' ? (
        <View style={styles.webMapFallback}>
          <View style={[styles.webMapHero, { backgroundColor: card, borderColor: inputBorder }]}> 
            <ThemedText style={styles.webMapTitle}>Map view is available on iOS and Android</ThemedText>
            <ThemedText style={{ color: muted, textAlign: 'center', marginTop: 8 }}>
              The browser uses the same nearby-tailor data in list mode. On mobile, the full interactive map loads here.
            </ThemedText>
            <Pressable
              onPress={() => setViewMode('list')}
              style={[styles.webMapButton, { backgroundColor: tint }]}
            >
              <ThemedText style={{ color: '#fff', fontWeight: '700' }}>Open List View</ThemedText>
            </Pressable>
          </View>
          <FlatList
            data={tailors}
